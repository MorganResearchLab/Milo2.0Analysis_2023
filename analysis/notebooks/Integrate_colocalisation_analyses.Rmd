...
write.table(gene.trait.pairs[, c("Gene", "PhenoPretty")],
            file="~/Dropbox/GLMM/results/GeneTraitPairs.tsv",
...
We know a lot more about the csQTLs that colocalise with an eQTL and a trait, but what about the 211 that just colocalise with a trait? These are interesting as they 
suggest either an alternative molecular QTL effect, or an impact upstream, such as on a cell fate decision.

## Trait colocalised, but no eQTL

```{r}
has.eqtl <- unique(comb.csqtls$csQTL)
has.trait <- unique(combined.coloc.df$csQTL)
just.trait <- setdiff(has.trait, has.eqtl)
```
...
just.trait.traits <- combined.coloc.df$PhenoDescription[combined.coloc.df$csQTL %in% just.trait]


immune.docs <- Corpus(VectorSource(gsub(just.trait.traits, pattern="_", replacement=" ")))
immune.docs <- immune.docs %>%
  tm_map(removeNumbers) %>%
  tm_map(removePunctuation) %>%
  tm_map(stripWhitespace)
...
set.seed(42) # for reproducibility 
png("~/Dropbox/GLMM/plots/Trait-andOnlyTrait_wordcloud.png", height=7, width=7, res=300, units="in")
wordcloud(words = immune.df$word, freq = immune.df$freq, min.freq = 1,
          max.words=200, random.order=FALSE, rot.per=0.35,
          colors=brewer.pal(8, "Dark2"))
dev.off()
