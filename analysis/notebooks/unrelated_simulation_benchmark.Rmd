---
title: "Unrelated data simulator"
output: html_notebook
---

I need to simulate unrelated individuals and their phenotypes to test the GLMM model without the kinship matrix.
...
runModel <- function(data.df, fe.var, re.var, glmm.model, ...){
    ...
    if(glmm.model %in% c("Milo")){
        ...
        model.coef <- c(coefficients(summary(model.run))$cond[, 1], unlist(lapply(summary(model.run)$varcor$cond, FUN=function(RX) RX[, 1])))
        ...
        model.coef <- tryCatch({
            suppressMessages({
                ...
                model.coef <- c(coefficients(summary(model.run))[, 1], as.numeric(VarCorr(model.run)[c(seq_along(re.var)), 1]))
            })
        }, error=function(err){
...
fe.benchmark.list <- sapply(seq_along(fe.data.list), FUN=function(iX, fe, re){
    DX <- fe.data.list[[iX]]
    milo.res <- runModel(DX, fe, re, "Milo")
    tmb.res <- runModel(DX, fe, re, "glmmTMB")
    pql.res <- runModel(DX, fe, re, "PQL")
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_FE1vary_FE2fix-Error.png",
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_FE1vary_FE2fix-estimate.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_FE2vary_FE1fix-Error.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_FE2vary_FE1fix-estimate.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_RE1Sigmavary_RE2-FE1-FE2fix-Error.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_RE1Sigmavary_RE2-FE1-FE2fix-Estimate.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_RE2Sigmavary_RE1-FE1-FE2fix-Error.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_RE2Sigmavary_RE1-FE1-FE2fix-Estimate.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_RE1Sigmavary-Sparsity_RE1-FE1-FE2fix-Error.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_FE1vary_RE1-FE2fix-pvalue.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_FE2vary_RE1-FE1fix-pvalue.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_Intercept_RE1-FE1fix-pvalue.png",
       height=3, width=5, dpi=300, bg='white')
```
...
ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_Intercept_RE1-FE2fix-pvalue.png",
       height=3, width=5, dpi=300, bg='white')
```
...
set.seed(NULL)
n.boots <- 100
fe.fdr.list <- sapply(seq_along(fe.data.list), FUN=function(iX, fe, re, nboots){
    DX <- fe.data.list[[iX]]
    
    boot.list <- list()
    for(i in seq_len(nboots)){
        iDX <- DX
        # iDX$Mean.Count <- sample(iDX$Mean.Count)
                
        for(x in seq_along(fe)){
            iDX[, fe[x]] <- sample(iDX[, fe[x]], size=nrow(iDX))
        }
        
        milo.res <- runModelPValue(iDX, fe, re, "Milo")
        tmb.res <- runModelPValue(iDX, fe, re, "glmmTMB")
        pql.res <- runModelPValue(iDX, fe, re, "PQL")

        all.res <- do.call(rbind.data.frame, list(milo.res, tmb.res, pql.res))
        colnames(all.res) <- c("Intercept", fe)
        all.res$Model <- c("Milo", "glmmTMB", "PQL")
        
        # summarise to get the number of false discoveries at different levels
        fdr.levels <- c(0.1, 0.05, 0.01)
        fdr.list <- list()
        for(q in seq_along(fdr.levels)){
            q.fdr <- fdr.levels[q]
            
            suppressMessages({
                q.all.fdr <- all.res %>% melt(id.vars=c("Model")) %>% 
                    group_by(Model, variable) %>% summarise(Sig=as.numeric(value < q.fdr))
                })
            q.all.fdr$FDRt <- q.fdr
            fdr.list[[q]] <- q.all.fdr
        }
        
        i.fdr.df <- do.call(rbind.data.frame, fdr.list)
        i.fdr.df$Boot <- i
        boot.list[[i]] <- i.fdr.df
    }
    
    boot.df <- do.call(rbind.data.frame, boot.list)
    
    # compute FDR as proportion of false discoveries
    suppressMessages({
        all.res <- boot.df %>% group_by(Model, variable, FDRt) %>% summarise(FDR=sum(Sig)/nboots)
    })
    ...
    return(all.res)
}, simplify=FALSE, fe=c("FE1", "FE2"), re=c("RE1"), nboots=n.boots)

fe.fdr.df <- do.call(rbind.data.frame, fe.fdr.list)
rownames(fe.fdr.df) <- NULL
```


```{r, fig.height=6, fig.width=7.5}
line.df <- data.frame("FDR"=c(0.1, 0.05, 0.01), "FDRt"=0.1, 0.05, 0.01)

ggplot(fe.fdr.df[fe.fdr.df$variable %in% c("FE2"),], 
       aes(x=ordered(FDRt), y=FDR, fill=Model, colour=Model)) +
    geom_hline(yintercept=0.1, lty=2, colour='grey20') +
    geom_hline(yintercept=0.05, lty=2, colour='grey50') +
    geom_hline(yintercept=0.01, lty=2, colour='grey80') +
    
    geom_boxplot() + 
    # geom_jitter(size=2.5, position=position_jitterdodge(jitter.width=0.1)) +
    
    theme_cowplot() +
    labs(x="FDR threshold", y="FDR") +
    expand_limits(y=c(0)) +
    facet_wrap(~FE2BETA, scales="free_y") +
    NULL

# ggsave("~/Dropbox/GLMM/plots/UnrelatedSim_benchmark_FDR.png",
#        height=3, width=5, dpi=300, bg='white')
```
