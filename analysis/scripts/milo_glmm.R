...
message("Diagnostic plotting")
pg <- plotNhoodSizeHist(big.mnn)
ggsave(pg, file=paste0(opt$output, "_NhoodHistogram.png"), dpi=300)

message("Running GLMM with ", opt$solver, " solver")
print(dim(meta.df))
print(dim(nhoodCounts(big.mnn)))
print(length(intersect(rownames(meta.df), colnames(nhoodCounts(big.mnn)))))
message("Formula: ", glmm.forumla)
glmm.res <- testNhoods(x=big.mnn, design=as.formula(glmm.formula), design.df=meta.df, reduced.dim=opt$reddim,
                       fdr.weighting="graph-overlap", glmm.solver=opt$solver, BPPARAM=param)
print(warnings())
print(dim(glmm.res))
