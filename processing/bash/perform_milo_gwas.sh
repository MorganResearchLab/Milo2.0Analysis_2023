...
## do a quick count of the number of SNPs to get ~10 SNPs per block.
A=$(cat $2.map | wc -l)
SNP=$(expr $A / 1000) # number of SNP per chunk - this is how many array jobs are needed.
...
bsub -q standard -M 40000 -R "rusage[mem=38000]" -T 120 -n 4 -J $1-Milo_GWAS_[1-$SNP] -e $BASE_DIR/logs/$1-Milo_GWAS_%I.err -o $BASE_DIR/logs/$1-Milo_GWAS_%I.out Rscript $SRC/milo_gwas.R --Milo $4 --fixed $FIXED --sampleID $SAMPID --reddim $REDIM  --GRM $3 --plink $2 --solver $SOLVER --maf $MAF --output $5 --chunk 1000
