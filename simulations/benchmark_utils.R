...
# Creates random differentially expressed regions over a dataset for benchmarking - assumes an additive genotype model
add_synthetic_labels_pop_by_genotype <- function(sce, # SingleCellExperiment obj
				                 pop, pop_column="celltype",
                                     		 pop_enr = 0.7,
                                     		 redDim='pca.corrected', # embedding to use to simulate differential abundance
                                     		 n_conditions=3, # number of genotype groups to simulate
  ...
  ## Normalize weights from enr_score to 1/3
  enr_scores <- rep(min.prob, ncol(w)) ## Generate enrichment prob for each cluster
  names(enr_scores) <- colnames(w)
  if(length(pop_enr) == length(pop)){
    enr_scores[pop] <- pop_enr
  } else{
  ...
  # altering the baseline probability can induce a skew towards a condition across _all_ cells
  geno1_prob <- sapply(1:ncol(w), function(i) .scale_to_range(w[,i], min=min.prob*condition_balance,
                                                              max=enr_scores[i]))
  geno2_prob <- sapply(1:ncol(w), function(i) .scale_to_range(w[,i], min=min.prob*condition_balance,
                                                              max=enr_scores[i]/2))
  colnames(geno1_prob) <- colnames(centroid_emb)
  colnames(geno2_prob) <- colnames(centroid_emb)
  
  # need to integrate over these to get the condition probabilities
  # need to set relevant pops only, force the others to ~1/3
  prob_geno1_matrix <- geno1_prob[, pop]
  prob_geno2_matrix <- geno2_prob[, pop]
  
  if(is(prob_geno1_matrix, "matrix")){
    geno1_probability <- rowMeans(prob_geno1_matrix)
    geno2_probability <- rowMeans(prob_geno2_matrix)
    for(x in seq_along(pop)){
      geno1_probability[sce[[pop_column]] == pop[x]] <- prob_geno1_matrix[sce[[pop_column]] == pop[x], pop[x]]
      geno2_probability[sce[[pop_column]] == pop[x]] <- prob_geno2_matrix[sce[[pop_column]] == pop[x], pop[x]]
    }
  } else{
    geno1_probability <- prob_geno1_matrix
    geno2_probability <- prob_geno2_matrix
  }
  
  ## Cap probabilities (to have the same number of DA cells w different maximum Fold Change)
  if (!is.null(cap_enr)) {
    geno1_probability <- ifelse(geno1_probability > cap_enr, cap_enr, geno1_probability)
    geno2_probability <- ifelse(geno2_probability > cap_enr, cap_enr/2, geno2_probability)
  }
